...
app = Flask(__name__)
CORS(app)  # Allows frontend fetch calls without CORS issues
...
def scan():
    try:
        ...
        result1 = subprocess.run(
            ["python", os.path.join(SRC_DIR, "live_capture.py")],
        ...
        # Dummy traffic data for line chart
        traffic_data = [
            {"time": "00:01", "count": 5},
            {"time": "00:02", "count": 2},
            {"time": "00:03", "count": 8},
        ...
        if anomaly_count == 0:
            anomaly_distribution = {
                "No Anomalies": 1  # To show a green pie if no anomalies
            }
        else:
            ddos = max(1, int(anomaly_count * 0.6))
...
if __name__ == "__main__":
    ...
    app.run(debug=True, use_reloader=False)  # Prevents infinite reload
